...
function generatePlantUML(s) {
  s = unescape(encodeURIComponent(s));
  const encoded = encode64(RawDeflate.deflate(s, 9));
  const src = `http://www.plantuml.com/plantuml/img/${encoded}`;
  return `<img src="${src}" />`
}
...
function setOutput(val) {
    ...
    val = val.replace(/(?:```plantuml[\s|\n]+)([^`]*)(?:[\s|\n]*```)/ig, function(a, b) {
        return generatePlantUML(b);
    });
